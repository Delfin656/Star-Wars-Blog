...
export const SinglePlanet = () => {
    ...
    const getSinglePlanet = async (pid) => {
        try {
            const response = await fetch(API_URL + `planets/${pid}`)
            if (!response.ok) {
                new Error("Ocurrió un error en la solicitud getSinglePlanet");
            }
        ...
        <div>
            <div className="d-flex">
                ...
                <div className="mx-4 justify-content-center bg-dark bg-gradient rounded">
                    <h1 className="text-warning mt-3 ms-3">{singlePlanet.name}</h1>
